...
cd inspec && bundle install && export PATH=$PATH:/inspec/inspec-bin/bin && rm /etc/pki/ca-trust/source/anchors/*
...
ENV CHEF_LICENSE="accept"
RUN update-ca-trust && microdnf module enable ruby:2.7 && microdnf install ruby openssh-clients && \
bundle install && export PATH=$PATH:/inspec/inspec-bin/bin && rm /etc/pki/ca-trust/source/anchors/*
CMD bash